function addChoicesForm(select){
	let bigQ = select.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.firstElementChild.firstElementChild.textContent;
	let Q = select.parentNode.parentNode.parentNode.parentNode.firstElementChild.firstElementChild.textContent;
	
	let inputCNum = select.parentNode.parentNode.parentNode.parentNode.lastElementChild;
	let choicesCnt = (Number(inputCNum.value) + 1);
	let choicesHtml = '<li class = "Select"><label><input type="radio" required name="Select_ans['+bigQ+']['+Q+']" value="'+choicesCnt+'"><span>'+choicesCnt+'．</span></label><input type="text" required class="Select_text" name="Select_text"/><button type="button" onclick="rmChoicesForm(this)" class = "Delete">−選択肢を削除</button></li>';
